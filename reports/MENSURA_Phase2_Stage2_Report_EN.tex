...
\large MENSURA --- Hypergraph Dimension Estimation, \\
...
``MENSURA'' toolkit for hypergraph-based physics, following
...
grounds: a higher-arity rule-search calculus (\emph{higher-arity rule search}) and an emergent-dimension estimator (\emph{Pre-Geometric
...
round. A third round then revised the goal to six of ten and met it (01,
02, 03, 04, 06, 10), but only after two further estimator-repair rounds, a
literature-grounded improvement loop, and a four-level review chain that
twice caught the \emph{reviewers} applying a genuine fairness fix to only
some of the problems it governed --- the same class of error round 2's own
correction diagnosed, now confirmed to affect reviewers as readily as
original claims. See Section~\ref{sec:round3}.
\end{abstract}
...
\begin{enumerate}
\item \textbf{Higher-arity rule search} (higher-arity algebra): extending
...
\section{Round 3: A Literature-Grounded Improvement Loop --- Goal Met}
\label{sec:round3}

A third round was launched immediately following the round 2 correction,
with a goal revised downward and sharpened: \textbf{six of ten problems,
via either the existing compute-savings criterion or a newly-formalized
accuracy criterion}, rather than eight of ten via compute savings alone.
Design, repair, and adjudication steps used a higher-tier model throughout.
\textbf{The revised goal was met: 6 of 10 problems (01, 02, 03, 04, 06, 10)
are verified wins}, after a four-level review chain --- self-report, audit,
two independent adversarial skeptics, and a dedicated closing pass --- that
twice caught the \emph{reviewers} repeating the same class of error found
in round 2.

\textbf{What changed.} Three literature-grounded hypotheses were pursued.
A Theiler-window exclusion (Theiler, 1986) was added to the traditional
baseline, correctly keyed on each point's original trajectory time index
rather than its position in a reordered array. A second win criterion,
asymptotic accuracy on a fixed large sample (criterion c), was formalized
in code so that the estimator's advantage on non-degenerate targets could
be measured rather than asserted in prose --- it produced zero verified
wins across two full rounds, and remains an open question about the
criterion's definition. A four-iteration AutoResearch-style loop (Karpathy,
2026) targeted the two chaotic attractors where the estimator needed
2--4$\times$ \emph{more} points than the baseline: two proposed code
changes were kept after measurably improving convergence (a saturation
guard on the fit window, and applying the Theiler correction to the
estimator's own graph construction), two were rejected after measurably
failing to help or causing a regression. A separate, still-open estimator
defect (near-constant shell sequences producing an unreliable fit-quality
statistic) required three repair rounds to close correctly; the fix that
finally held is backed by a proof that the affected estimate is
structurally confined to a bounded interval, not merely empirically clean
on the cases tested.

\textbf{The result that decided the round, however, was procedural.} The
adjudicator discovered that the traditional baseline's correlation-sum
radius window had been hard-coded and was materially misconfigured on
several targets --- a fairness bug in the \emph{comparator}, not the
estimator, that had quietly affected every round's numbers. Correcting it
changed real verdicts. But the correction was first applied only to the
problems where a fairer baseline \emph{removed} a poly win, producing a
self-consistent-looking 4-of-10 that read as careful skepticism. A
dedicated skeptic, instructed specifically to check whether a stated
correction had been applied everywhere it logically should have been ---
the exact discipline this report's own round 2 correction established ---
found the same fairness fix, applied to the remaining problems, added wins
it had not yet been permitted to add. A final closing pass, applying the
correction symmetrically and with the same full stable-convergence rule
used throughout this programme, settled the count at 6 of 10.

\textbf{This must be read with three caveats.} First, five of the six wins
are the same ring-lattice degeneracy mechanism reported in round 2, now
scoped further: it requires \emph{deterministic}, not merely uniform-density,
sampling of a smooth closed orbit --- on an identical manifold sampled
i.i.d.\ at random, the result inverts completely. Second, every headline
savings figure is smaller once the baseline is fairly tuned: the round's
largest win is no longer the harmonic oscillator's originally-reported
0.875 but planar Brownian motion at \textbf{0.969} --- and Brownian motion
is also the only win in either round that is not a ring-lattice artifact,
with a genuine, non-degenerate accuracy advantage. Third, all five
ring-lattice wins lose their advantage at a Theiler window of roughly two
sample spacings, a shared fragility rather than a per-problem one. Full
scoreboard, mechanism analysis, and the four-level review chain are in
...
\item[] Karpathy, A. (2026). \emph{AutoResearch}: an agentic
  propose-measure-keep loop for autonomous empirical improvement.
